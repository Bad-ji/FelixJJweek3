...
public class Search {
	...
	public Search(int lenght, int[] array) {
		super();
		this.lenght = lenght;
		this.array = array;
	}
	...
	public String toString() {
		return "Search [lenght=" + lenght + ", array=" + Arrays.toString(array) + "]";
	}
	...
	public boolean dichotoSearch(int val,SortType type) {
		int left =0;
		...
		while(left<=right && !find) {
			...
			if(Features.test(val,this.array[medium], type)) {
				left=medium+1;
			}
			else {
				right=medium-1;
	...
	public  boolean fibonnaciSearch(int val,SortType type){
		...
		while (numberfibonnaci > 1) {
			...
			if (Features.test(val,this.array[i],type)) {
			...
			else if (!Features.test(val,this.array[i],type) && val!=this.array[i]) {
	...
	public boolean searchByJump(int val,SortType type) {
		
        int jump = (int) Math.sqrt(this.lenght);
        
        int p = 0;
        
        while (Features.test(val,this.array[Math.min(jump, this.lenght)-1] ,type))
        {
            p = jump;
